...
vec2 formula(vec2 uv, float freq, float ampl, float vel, int modo, float fase, float cant) {
  ...
  if (modo == 0) {
	...
	float ang = atan(pos.x,pos.y);
        ...
        float freq2 = freq*0.01;

        float forma = 1.0 - sm(freq2, freq2+ampl, rad+fase);
	forma*=sin(uv.x * freq + time * vel + fase) * ampl;
              resultado = vec2(forma); 
    }
    if (modo == 11) {
	vec2 pos = vec2(PosX, PosY) - uv;
	...
	float ang = atan(pos.x,pos.y);
        ...
        float forma = 1.0 - sm(freq2, freq2+ampl, rad+fase);
	forma*=sin(uv.y * freq + time * vel + fase) * ampl;
              resultado = vec2(forma); 
    }
     if (modo == 12) {
	vec2 pos = vec2(PosX, PosY) - uv;
	...
	float ang = atan(pos.x,pos.y);
        ...
        float forma = 1.0 - sm(freq2, freq2+ampl, rad+fase);
	forma*=sin((uv.x+uv.y) * freq + time * vel + fase) * ampl; // puede ser tan cos
              resultado = vec2(forma); 
    }
 ...
 if (modo == 13) {
	...
	float ang = atan(pos.x,pos.y);
        ...
        float forma = 1.0 - sm(freq2, freq2+ampl, rad+fase);
	forma*= tan(uv.x * freq + time * vel + fase) * ampl;
              resultado = vec2(forma); 
    }
    if (modo == 14) {
	vec2 pos = vec2(PosX, PosY) - uv;
	...
	float ang = atan(pos.x,pos.y);
        ...
        float forma = 1.0 - sm(freq2, freq2+ampl, rad+fase);
	forma*= tan(uv.y * freq + time * vel + fase) * ampl;
              resultado = vec2(forma); 
    }
     if (modo == 15) {
	vec2 pos = vec2(PosX, PosY) - uv;
	...
	float ang = atan(pos.x,pos.y);
        ...
        float forma = 1.0 - sm(freq2, freq2+ampl, rad+fase);
	forma*=tan((uv.x+uv.y) * freq + time * vel + fase) * ampl; // puede ser tan cos
              resultado = vec2(forma); 
    }
